...
export const SolutionStatusManager: React.FC<SolutionStatusManagerProps> = ({
  ...
  // Normalize status values to handle case mismatches from API
  const normalizeStatus = (status: string): SolutionStatus => {
    ...
    return statusMap[status] || SolutionStatus.PENDING;
  };
  ...
  const [selectedStatus, setSelectedStatus] = useState(normalizedCurrentStatus);
  ...
  // Default fallback configuration for invalid or undefined status
  const defaultConfig = {
  ...
  // Get current configuration with fallback for undefined status
  const currentConfig = statusConfig[normalizedCurrentStatus] || defaultConfig;
        ...
        <div className="absolute top-full left-0 mt-2 w-80 bg-surface border border-border rounded-xl shadow-xl z-50 p-4">
          ...
          <div className="space-y-2 mb-4">
            {Object.entries(statusConfig).map(([status, config]) => {
              // Safety check to ensure config is defined
