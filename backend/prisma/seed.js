...
async function main() {
    ...
    const passwordHash = await bcryptjs_1.default.hash('DroneWERX2024!', 12);
    ...
    const users = await Promise.all([
        prisma.user.create({
            data: {
                ...
                role: client_1.UserRole.WARFIGHTER,
                passwordHash,
                isVerified: true,
        ...
        prisma.user.create({
            data: {
                ...
                role: client_1.UserRole.INNOVATOR,
                passwordHash,
                isVerified: true,
    ...
    const threads = await Promise.all([
        prisma.thread.create({
            data: {
                ...
                urgency: client_1.Urgency.IMMEDIATE,
                trlLevel: client_1.TRLLevel.TRL4,
                domain: 'Urban Operations',
                location: 'Urban Combat Training Center',
                classification: client_1.ClassificationLevel.PUBLIC,
                isAnonymous: false,
                views: 234,
                upvotes: 18,
                downvotes: 2,
        ...
        prisma.thread.create({
            data: {
                ...
                status: client_1.ThreadStatus.IN_PROGRESS,
                urgency: client_1.Urgency.PRIORITY,
                trlLevel: client_1.TRLLevel.TRL3,
        ...
        prisma.thread.create({
            data: {
                ...
                status: client_1.ThreadStatus.OPEN,
                urgency: client_1.Urgency.PRIORITY,
                trlLevel: client_1.TRLLevel.TRL2,
                domain: 'Special Operations',
                location: 'Classified',
                classification: client_1.ClassificationLevel.RESTRICTED,
                isAnonymous: true,
                views: 89,
                upvotes: 15,
                downvotes: 0,
        ...
        prisma.thread.create({
            data: {
                ...
                priority: client_1.Priority.CRITICAL,
                status: client_1.ThreadStatus.OPEN,
                urgency: client_1.Urgency.FLASH,
                trlLevel: client_1.TRLLevel.TRL5,
                domain: 'Combat Operations',
                location: 'Field Operations Center',
                classification: client_1.ClassificationLevel.CONFIDENTIAL,
                isAnonymous: false,
                views: 312,
                upvotes: 28,
                downvotes: 3,
        ...
        prisma.thread.create({
            data: {
                ...
                status: client_1.ThreadStatus.SOLVED,
                urgency: client_1.Urgency.ROUTINE,
                trlLevel: client_1.TRLLevel.TRL6,
    ...
    await Promise.all([
        prisma.threadTag.create({ data: { threadId: threads[0].id, tagId: tags[0].id } }),
        prisma.threadTag.create({ data: { threadId: threads[0].id, tagId: tags[1].id } }),
        prisma.threadTag.create({ data: { threadId: threads[0].id, tagId: tags[5].id } }),
        prisma.threadTag.create({ data: { threadId: threads[1].id, tagId: tags[3].id } }),
        prisma.threadTag.create({ data: { threadId: threads[1].id, tagId: tags[7].id } }),
        prisma.threadTag.create({ data: { threadId: threads[2].id, tagId: tags[2].id } }),
        prisma.threadTag.create({ data: { threadId: threads[2].id, tagId: tags[7].id } }),
        prisma.threadTag.create({ data: { threadId: threads[3].id, tagId: tags[4].id } }),
        prisma.threadTag.create({ data: { threadId: threads[3].id, tagId: tags[5].id } }),
        prisma.threadTag.create({ data: { threadId: threads[3].id, tagId: tags[1].id } }),
        prisma.threadTag.create({ data: { threadId: threads[4].id, tagId: tags[9].id } }),
        prisma.threadTag.create({ data: { threadId: threads[4].id, tagId: tags[6].id } }),
    ]);
    ...
    const solutions = await Promise.all([
        ...
        prisma.solution.create({
            data: {
                ...
                trlLevel: client_1.TRLLevel.TRL7
            }
        }),
        prisma.solution.create({
            data: {
                ...
                trlLevel: client_1.TRLLevel.TRL8
            }
        })
    ]);
